...
class AuthController with ChangeNotifier {
  ...
  Future registration(RegisterModel register, File image, Function callback) async {
    _isLoading = true;
    notifyListeners();

    final url = 'https://bharatoutletstore.com/api/v1/auth/register';
    ...
    try {
      ...
      if (await image.exists()) {
        ...
        print("Image does not exist at the provided path: ${image.path}");
      }

      // Create FormData with registration fields and the image file
      var formData = FormData.fromMap({
        ...register.toJson(), // Adds registration fields directly into formData
        'billImage': await MultipartFile.fromFile(image.path, filename: 'shop_image.jpg'),
      });

      // Debugging: Print FormData fields to ensure image is added
      formData.fields.forEach((field) {
        print('FormData field: ${field.key}, value: ${field.value}');
      });
      ...
      if (response.statusCode == 200) {
        ...
        callback(false, null, null, 'Registration failed');
      }
    } on DioError catch (error) {
      _isLoading = false;
      notifyListeners();
      print('DioError: ${error.response?.data ?? error.message}');
      callback(false, null, null, 'Something went wrong');
    } catch (error) {
      _isLoading = false;
      notifyListeners();
